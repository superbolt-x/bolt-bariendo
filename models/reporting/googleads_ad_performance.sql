...
LEFT JOIN 
    (SELECT 'day' as date_granularity,
        ...
        COALESCE(SUM(CASE WHEN (conversion_action_name ~* 'signup' or conversion_action_name ~* 'Strawberry') THEN conversions END),0) as signups,
        COALESCE(SUM(CASE WHEN (conversion_action_name ~* 'Consultation Payment' or conversion_action_name ~* 'Cherry') THEN conversions END),0) as consultation_payment
